...
## 1. Het schema
- [ ] 1.1 `netnl-findings/v1` schrijven zoals de consumentkant hem
  ...
  `detail`.
- [ ] 1.2 `category` afleiden uit de testnaam: het langste voorvoegsel
...
- [ ] 1.3 `measured_at` = `request.finished_date` van de batch, voor
...
- [ ] 1.4 `detail` is altijd `null` in v1. De batch-API levert geen
...
- [ ] 2.1 Een batch die niet `done` is: exitcode niet-nul, geen
...
- [ ] 2.2 Een domein met `status != "ok"` (bijv. `error`) komt in het
  ...
  batch zat.
- [ ] 2.3 Tests: een lopende batch, een mislukte batch, een batch met
...
- [ ] 3.1 Stabiele volgorde (domeinen alfabetisch, tests alfabetisch)
  en stabiele JSON-opmaak, zodat twee exports van dezelfde batch
  byte-identiek zijn. Test die dat vastlegt.
- [ ] 3.2 De twee echte metingen als gouden fixtures opnemen:
...
- [ ] 3.3 Een test die de export van de web-fixture vergelijkt met het
  verwachte findings-bestand, en hetzelfde voor mail.

## 4. Afronden
- [ ] 4.1 README: één alinea over de uitvoervorm en waar het schema
  staat. CHANGELOG onder [Unreleased].
- [ ] 4.2 Het contract (`NETNL-CONTRACT.md` uit de Wanderer-change)
